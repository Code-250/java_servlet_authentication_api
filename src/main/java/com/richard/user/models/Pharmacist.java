...
public class Pharmacist extends User {
  ...
  public ApiResponse<User> signup() throws Exception {
    if (Users.findUser(getUserName()) != null) {
      throw new Exception(" pharmacisty already exists");
    }
    if (!Pattern.matches("^(?=.*[0-9])(?=.*[a-zA-Z])(?=\\S+$).{6,}$", getPassword())) {
      throw new Exception("Password rule does not match");
    }
    encryptPassword();
    Users.addUser(this);
    return new ApiResponse<>("Pharmacist signup Successfully", Users.findUser(getUserName()));
  }
